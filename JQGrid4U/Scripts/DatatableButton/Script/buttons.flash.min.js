(function (g) { "function" === typeof define && define.amd ? define(["jquery", "datatables.net", "datatables.net-buttons"], function (j) { return g(j, window, document) }) : "object" === typeof exports ? module.exports = function (j, l) { j || (j = window); if (!l || !l.fn.dataTable) l = require("datatables.net")(j, l).$; l.fn.dataTable.Buttons || require("datatables.net-buttons")(j, l); return g(l, j, j.document) } : g(jQuery, window, document) })(function (g, j, l, p) {
	function v(a) {
		for (var b = ""; 0 <= a;)b = String.fromCharCode(a % 26 + 65) + b, a = Math.floor(a / 26) -
			1; return b
	} function n(a, b, d) { var c = a.createElement(b); d && (d.attr && g(c).attr(d.attr), d.children && g.each(d.children, function (a, b) { c.appendChild(b) }), null !== d.text && d.text !== p && c.appendChild(a.createTextNode(d.text))); return c } function B(a, b) {
	...
	q === p && (q = -1 === u.serializeToString(g.parseXML(o["xl/worksheets/sheet1.xml"])).indexOf("xmlns:r")); g.each(a, function (b, d) {
		...
		y = function (a) { return a.newline ? a.newline : navigator.userAgent.match(/Windows/) ? "\r\n" : "\n" }, z = function (a, b) {
			...
			new function () { this.serializeToString = function (a) { return a.xml } } : new XMLSerializer, q, o = {
		...
		A = [{ match: /^\-?\d+\.\d%$/, style: 60, fmt: function (a) { return a / 100 } }, { match: /^\-?\d+\.?\d*%$/, style: 56, fmt: function (a) { return a / 100 } }, { match: /^\-?\$[\d,]+.?\d*$/, style: 57 }, { match: /^\-?£[\d,]+.?\d*$/, style: 58 }, { match: /^\-?€[\d,]+.?\d*$/, style: 59 }, { match: /^\([\d,]+\)$/, style: 61, fmt: function (a) { return -1 * a.replace(/[\(\)]/g, "") } }, { match: /^\([\d,]+\.\d{2}\)$/, style: 62, fmt: function (a) { return -1 * a.replace(/[\(\)]/g, "") } }, { match: /^[\d,]+$/, style: 63 }, { match: /^[\d,]+\.\d{2}$/, style: 64 }]; h.Buttons.swfPath = "//cdn.datatables.net/buttons/" +
			h.Buttons.version + "/swf/flashExport.swf"; h.Api.register("buttons.resize()", function () { g.each(i.clients, function (a, b) { b.domElement !== p && b.domElement.parentNode && b.positionElement() }) }); h.ext.buttons.copyFlash = g.extend({}, t, {
				...
				className: "buttons-excel buttons-flash", text: function (a) { return a.i18n("buttons.excel", "Excel") }, action: function (a, b, d, c) {
					this.processing(!0); var a = c._flash, e = 0, f = g.parseXML(o["xl/worksheets/sheet1.xml"]), i = f.getElementsByTagName("sheetData")[0], d = {
						_rels: { ".rels": g.parseXML(o["_rels/.rels"]) }, xl: {
							_rels: { "workbook.xml.rels": g.parseXML(o["xl/_rels/workbook.xml.rels"]) },
							"workbook.xml": g.parseXML(o["xl/workbook.xml"]), "styles.xml": g.parseXML(o["xl/styles.xml"]), worksheets: { "sheet1.xml": f }
						}, "[Content_Types].xml": g.parseXML(o["[Content_Types].xml"])
					}, k = b.buttons.exportData(c.exportOptions), j, l, h = function (a) {
						j = e + 1; l = n(f, "row", { attr: { r: j } }); for (var b = 0, c = a.length; b < c; b++) {
							var d = v(b) + "" + j, h = null; if (!(null === a[b] || a[b] === p || "" === a[b])) {
							a[b] = g.trim(a[b]); for (var k = 0, m = A.length; k < m; k++) {
								var o = A[k]; if (a[b].match && !a[b].match(/^0\d+/) && a[b].match(o.match)) {
									h = a[b].replace(/[^\d\.\-]/g,
										""); o.fmt && (h = o.fmt(h)); h = n(f, "c", { attr: { r: d, s: o.style }, children: [n(f, "v", { text: h })] }); break
								}
							} h || ("number" === typeof a[b] || a[b].match && a[b].match(/^-?\d+(\.\d+)?$/) && !a[b].match(/^0\d+/) ? h = n(f, "c", { attr: { t: "n", r: d }, children: [n(f, "v", { text: a[b] })] }) : (o = !a[b].replace ? a[b] : a[b].replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g, ""), h = n(f, "c", { attr: { t: "inlineStr", r: d }, children: { row: n(f, "is", { children: { row: n(f, "t", { text: o }) } }) } }))); l.appendChild(h)
							}
						} i.appendChild(l); e++
					}; g("sheets sheet", d.xl["workbook.xml"]).attr("name",
